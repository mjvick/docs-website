...
### Regression tag [#regressions-tag]
...
### Expected errors tags [#expected-tags]

An error group is tagged with an "expected" tag if all error occurrences in the group are expected. It is tagged with a "contains expected errors" tag if some error occurrences are expected and some are not.
See more on how to [mark errors as expected](/docs/apm/agents/manage-apm-agents/agent-data/manage-errors-apm-collect-ignore-or-mark-expected/#expected).

<img
  title="Screenshot of expected tag text."
  alt="Screenshot of expected tag text."
  src={errorsinboxExpectedErrors}
/>
<img
  title="Screenshot of contains expected tag text."
  alt="Screenshot of contains expected tag text."
  src={errorsinboxContainsExpectedErrors}
/>
...
Errors inbox enables you to filter out expected errors. You can [mark errors as expected](/docs/apm/agents/manage-apm-agents/agent-data/manage-errors-apm-collect-ignore-or-mark-expected/#expected), and filter them out of your inbox to remove noise.

* `error.expected != true`: This is the default. It hides all error groups that have only expected errors and it marks error groups that have a mix of expected and unexpected errors with the "contains expected errors" tag.
...
* Removing the error.expected filter will show all error groups whether they are marked as expected or not.
